#!/bin/bash
#脚本支持以 self_compression.sh [Source compressed file] [Destination path]的格式对目标文件进行解压缩，并且支持task3.sh --list的形式查询脚本可解压缩的文件类型
...
if [ ! -n "$1" ] ;then                #无传参是输出usage
    echo -e "usage: self_compression.sh [--list] or [Source compressed file] [Destination path] \nSelf compression according to the file name suffix"
